    ...
    <div className="min-h-screen bg-[#09090b] text-zinc-100 flex flex-col">
      <div className="w-full max-w-[1400px] mx-auto px-6 py-6 space-y-6 flex-1">
        ...
        <div className="lg:col-span-8 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="px-3 py-1.5 bg-[#A3E635] text-black hover:bg-[#A3E635]/90 transition text-xs font-bold rounded-lg flex items-center gap-1 cursor-pointer"
            >
          ...
          <div className="border border-zinc-800 rounded-xl overflow-hidden bg-[#121215]/40">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="border-b border-zinc-800/60 bg-[#09090b]/50 text-[10px] text-zinc-500 uppercase font-mono tracking-wider font-extrabold select-none">
              ...
              <tbody className="divide-y divide-zinc-850">
                {rules.map((r) => (
                  <tr key={r.id} className="hover:bg-zinc-900/30 transition duration-150">
                    <td className="py-3.5 px-4">
                      <div className="flex flex-col gap-0.5">
                        <span className="font-semibold text-zinc-100 font-sans">{r.name}</span>
                    ...
                    <td className="py-3.5 px-4 font-mono font-semibold text-zinc-300">{r.service}</td>
                    <td className="py-3.5 px-4 font-mono text-zinc-400">{r.duration}s</td>
                    <td className="py-3.5 px-4 font-mono text-[#A3E635] font-bold">≥{r.threshold.toFixed(2)}</td>
                    <td className="py-3.5 px-4 text-center">
                      <button
                        onClick={() => handleToggleRule(r.id)}
                        className="text-zinc-400 hover:text-white transition cursor-pointer inline-flex items-center justify-center"
                    ...
                    <td className="py-3.5 px-4 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        <button
                          ...
                          className="p-1 hover:bg-zinc-850 rounded text-rose-450 hover:text-rose-400 transition cursor-pointer"
                          title="Delete Rule"
                        >
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="card bg-[#121215] border border-zinc-800 rounded-xl p-5 shadow-lg space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs font-mono">
              ...
              <div className="text-zinc-300 font-semibold">{user?.full_name}</div>
              <div className="text-zinc-500">Email</div>
              <div className="text-zinc-300 truncate">{user?.email}</div>
              <div className="text-zinc-500">User ID</div>
              <div className="text-zinc-400 font-bold truncate">{user?.id}</div>
            </div>
          </div>
          ...
          <div className="card bg-[#121215] border border-zinc-800 rounded-xl p-5 shadow-lg space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs font-mono">
              <div className="text-zinc-500">Name</div>
              <div className="text-zinc-300 font-semibold">{currentOrg?.name}</div>
              <div className="text-zinc-500">Slug</div>
              <div className="text-zinc-300">{currentOrg?.slug}</div>
              <div className="text-zinc-500">Role</div>
              <div className="text-zinc-300">{currentOrg?.role}</div>
              <div className="text-zinc-500">Org ID</div>
              <div className="text-zinc-400 font-bold truncate">{currentOrg?.id}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm animate-fade-in">
          ...
          />
          <div className="relative w-full max-w-md p-6 bg-[#121215] border border-zinc-800 rounded-2xl shadow-2xl flex flex-col gap-4 font-sans text-zinc-300">
            <button
              onClick={() => setModalOpen(false)}
            ...
            <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-1.5 pb-2 border-b border-zinc-800">
            ...
            <form onSubmit={handleSaveRule} className="space-y-4 text-xs">
              <div className="flex flex-col gap-1.5">
                <label className="font-bold text-zinc-500 uppercase tracking-widest text-[9px]">Rule Name</label>
                <input
                  type="text"
                  required
                  placeholder="e.g. Rate-limit payment outage burst"
                  className="w-full bg-[#09090b] border border-zinc-800 rounded-lg py-2 px-3 focus:border-[#A3E635]/65 outline-none text-zinc-200"
                  value={formName}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="font-bold text-zinc-500 uppercase tracking-widest text-[9px]">Service Target</label>
                  <input
                    type="text"
                    required
                    placeholder="e.g. payment-api"
                    className="w-full bg-[#09090b] border border-zinc-800 rounded-lg py-2 px-3 focus:border-[#A3E635]/65 outline-none text-zinc-200 font-mono"
                    value={formService}
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="font-bold text-zinc-500 uppercase tracking-widest text-[9px]">Severity Level</label>
                  <select
                    className="w-full bg-[#09090b] border border-zinc-800 rounded-lg py-2 px-3 focus:border-[#A3E635]/65 outline-none text-zinc-200 cursor-pointer"
                    value={formSeverity}
              ...
              <div className="flex items-center justify-between border-t border-zinc-800 pt-3 mt-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-zinc-400 font-bold uppercase tracking-wider text-[10px]">Rule Active</span>
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => setModalOpen(false)}
                    className="px-3.5 py-1.5 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-350 hover:text-white font-bold rounded-lg transition cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-3.5 py-1.5 bg-[#A3E635] text-black hover:bg-[#A3E635]/90 font-bold rounded-lg transition cursor-pointer shadow-lg"
                  >
                    Save Rule
                  </button>
                </div>
